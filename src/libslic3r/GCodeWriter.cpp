...
std::string GCodeWriter::toolchange(unsigned int filament_id, int nozzle_id)
{
    ...
    if (this->multiple_extruders || (this->config.filament_diameter.values.size() > 1 && !is_bbl_printers())) {
        ...
        // Orca-managed mapping: a logical filament/colour switch that stays on the same physical
        // extruder is ;VT metadata only — it must not emit a real tool change.
        ...
        if (physical_tool_changed) {
            ...
            if (this->m_is_bbl_printers && !config.manual_filament_change)
...
std::string GCodeWriter::_travel_to_z(double z, const std::string &comment)
{
    ...
    // Orca: m_cached_extruder_idx defaults to 0 and is refreshed on every toolchange, so this no
    // longer dereferences filament() (null until the first toolchange). That matters for the silent
    // Z-offset move GCode::preamble() emits on non-BBL printers, where init_extruder() has not run.
...
{
    std::string output;
    // Orca: see _travel_to_z — m_cached_extruder_idx avoids the pre-first-toolchange null filament().
    double speed = this->config.travel_speed_z.get_at(m_cached_extruder_idx);
