...
// Only add a newline in case the current G-code does not end with a newline.
    ...
    std::string WipeTowerIntegration::append_tcr(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_filament_id, double z) const
    {
        ...
        if (! change_filament_gcode.empty()) {
            ...
            // Orca-managed mapping: the tool-related placeholders carry mapped output tool ids,
            // not raw filament ids.
            config.set_key_value("previous_extruder",
                new ConfigOptionInt(old_filament_id >= 0 ? int(gcodegen.get_toolchange_id(old_filament_id)) : -1));
            ...
            // BBS
            {
                ...
                // use_interface_temp additionally requires the incoming filament to be a support
                // filament — without that, the interface preheat also fired for non-support filaments.
                if (use_interface_temp) {
        ...
        // Match against the mapped output tool id — that is what the emitted T command carries.
        ...
        // Always return to the wipe tower after any real toolchange, even when change_filament_gcode
        // is empty (e.g. non-BBL multi-extruder printers that only emit a T command).
        if (!need_travel_after_change_filament_gcode && !toolchange_gcode_str.empty())
            need_travel_after_change_filament_gcode = true;

        if (need_travel_after_change_filament_gcode) {
            // move to start_pos for wiping after toolchange
            if (!is_used_travel_avoid_perimeter) {
                // Use external MP so the return path to the tower goes around (not through) the model.
                gcodegen.m_avoid_crossing_perimeters.use_external_mp_once();
        ...
        // Orca-managed mapping: current_extruder is the mapped output tool id.
        ...
        if (!filament_start_gcode.empty()) {
            ...
            DynamicConfig config;
            // Orca-managed mapping: mapped output tool id, not the raw filament id.
    ...
    std::string WipeTowerIntegration::tool_change(GCode &gcodegen, int extruder_id, bool finish_layer)
    {
        ...
        if (m_layer_idx >= (int) m_tool_changes.size())
            return gcode;
        // Unified wipe tower injection for all printer types.
...
//        print_object_instances_ordering = sort_object_instances_by_max_z(print);
    ...
    // Orca-managed mapping: the tool/extruder placeholders carry mapped output tool ids.
    ...
    if (is_bbl_printers) {
        ...
        // add the missing filament start gcode in machine start gcode
        {
            ...
            // Mark the initial logical filament for the preview parser. Multi-nozzle printers (H2C)
            // additionally carry " H%d" (dynamic ? nozzle_id : -1); single-nozzle printers keep the
            // bare marker so their g-code is unchanged. Both consumers accept the ";VT T<n>" spelling.
...
*/
    ...
    if (print.calib_params().mode == CalibMode::Calib_PA_Line) {
        ...
        if (NOZZLE_CONFIG(outer_wall_acceleration) > 0) {
            gcode += m_writer.set_print_acceleration(this->rounded_filament_acceleration(NOZZLE_CONFIG(outer_wall_acceleration)));
        }
    ...
    const bool                               prime_extruder)
{
    ...
    if (first_layer) {
        ...
        if (NOZZLE_CONFIG(default_acceleration) > 0 && NOZZLE_CONFIG(initial_layer_acceleration) > 0) {
            gcode += m_writer.set_print_acceleration(this->rounded_filament_acceleration(NOZZLE_CONFIG(initial_layer_acceleration)));
        }
    ...
    if (!first_layer && !m_second_layer_things_done) {
      ...
      if (NOZZLE_CONFIG(default_acceleration) > 0 && NOZZLE_CONFIG(initial_layer_acceleration) > 0) {
        gcode += m_writer.set_print_acceleration(this->rounded_filament_acceleration(NOZZLE_CONFIG(default_acceleration)));
      }
...
std::string GCode::travel_to(const Point& point, ExtrusionRole role, std::string comment, double z/* = DBL_MAX*/)
{
    ...
    if (this->on_first_layer()) {
        const double initial_layer_travel_acceleration = m_config.get_abs_value_at("initial_layer_travel_acceleration", get_nozzle_config_index(m_writer.filament()->id()));
        const double initial_layer_travel_jerk         = m_config.get_abs_value_at("initial_layer_travel_jerk", get_nozzle_config_index(m_writer.filament()->id()));

        if (NOZZLE_CONFIG(default_acceleration) > 0 && initial_layer_travel_acceleration > 0)
            acceleration_to_set = this->rounded_filament_acceleration(initial_layer_travel_acceleration);
        ...
        const bool short_perimeter_travel =
            is_short_travel && (is_perimeter(role) || (role == erNone && is_perimeter(m_last_processor_extrusion_role)));
        ...
        if (NOZZLE_CONFIG(default_acceleration) > 0) {
            if (short_perimeter_travel) {
                if (m_config.travel_short_distance_acceleration.value > 0) {
                    ...
                    bool used_bridge_acceleration = false;
                    if (role == erOverhangPerimeter) {
                        ...
                        if (bridge_acceleration > 0) {
                            used_bridge_acceleration = true;
                            acceleration_to_set = this->rounded_filament_acceleration(bridge_acceleration);
                        }
                    }
                    if (!used_bridge_acceleration && NOZZLE_CONFIG(outer_wall_acceleration) > 0) {
                        // Keep legacy fallback behavior when short-travel acceleration is disabled.
                        acceleration_to_set = this->rounded_filament_acceleration(NOZZLE_CONFIG(outer_wall_acceleration));
                    }
                }
            } else {
                if (NOZZLE_CONFIG(travel_acceleration) > 0)
                    acceleration_to_set = this->rounded_filament_acceleration(NOZZLE_CONFIG(travel_acceleration));
            }
        }

        if (NOZZLE_CONFIG(default_jerk) > 0) {
            if (short_perimeter_travel && NOZZLE_CONFIG(outer_wall_jerk) > 0) {
