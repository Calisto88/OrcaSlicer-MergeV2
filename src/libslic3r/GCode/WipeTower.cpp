...
<<<<<<< HEAD
        ...
        // (inert unless is_valid_last_layer clamps, i.e. multi-extruder near Z-max). This runs from
        // get_all_wall_skip_points(), which pre-computes every layer, so it uses the layer_id
        // parameter rather than m_cur_layer_id (which only tracks the layer being generated).
        if (!is_valid_last_layer(old_filament, layer_id, layer.z))
            nozzle_change_depth = 0.f;
...
<<<<<<< HEAD
    ...
    // Must follow set_nozzle_last_layer_id(): get_wall_skip_points() consults is_valid_last_layer().
    if (m_use_gap_wall)
