...
void GCodeProcessor::run_post_process()
{
            ...
            if (tool_number != -1) {
                ...
                int override_temp = -1;
                {
                    if (line_count > 0) {
                        ...
                        if (in_block) {
                            int base_temp = int(current_layer_id > 1 ? m_filament_nozzle_temp[tool_number] :
                ...
                export_line.insert_lines(
                    backtrace, cmd,
                    // line inserter
                    [tool_number, override_temp, this, &current_layer_id](unsigned int id, const std::vector<float>& time_diffs) {
                        ...
                        int temperature = override_temp > 0 ? override_temp : int(current_layer_id > 1 ? m_filament_nozzle_temp[tool_number] :
                                                                                                    m_filament_nozzle_temp_first_layer[tool_number]);
    ...
    auto build_usage_blocks = [&](const std::string& gcode_line, int line_id) {
        ...
        if (GLine::cmd_starts_with(gcode_line, ";VT")) {
            ...
            // Orca-managed mapping spells the marker ";VT T<fid>"; accept the optional 'T' so this
            // path sees the same changes process_VT() does (which strips it the same way).
            if (str >> std::ws && (str.peek() == 'T' || str.peek() == 't'))
                str.get();
            int fid;
            str >> fid;
            if (!str.fail() && 0 <= fid && fid < 255) {
...
void GCodeProcessor::apply_config(const DynamicPrintConfig& config)
{
    ...
    // Bed-type-specific Z offset: supersedes upstream's plain "z_offset" read.
    m_z_offset = static_cast<float>(get_active_z_offset(config));
    ...
    if (command.length() > 1) {
        if (eid < 0 || eid > 254) {
            ...
            if (uses_physical_tool_ids() &&
                ...
                m_pending_logical_filament_id = -1;
            // Pass the resolved logical filament id, not the raw tool id: under Orca-managed mapping
            // eid is a physical tool number, and outside it target_filament_id == eid anyway.
            ...
            process_filament_change(target_filament_id, nozzle_id);
        }
    }
}
