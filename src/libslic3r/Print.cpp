...
{
    // Orca-managed mapping: normalize the incoming map before it reaches the config, so a
    // single-physical-extruder multi-colour setup collapses every filament to mapping 1.
...
ExtrusionLayers FakeWipeTower::getTrueExtrusionLayersFromWipeTower() const
{
    ...
    for (auto it = outer_wall.begin(); it != outer_wall.end(); ++it) {
        ...
        for (auto &polyline : it->second) {
            ...
            path.polyline = Polyline3(transform_outer_wall_polyline(polyline));
