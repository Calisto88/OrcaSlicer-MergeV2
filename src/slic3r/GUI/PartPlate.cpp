...
static Slic3r::DynamicPrintConfig full_config_for_current_mapping_mode(Slic3r::PresetBundle *preset_bundle, Slic3r::GUI::PartPlate *part_plate, const Slic3r::DynamicConfig &project_config)
{
    ...
    std::vector<int> filament_maps = part_plate->get_real_filament_maps(project_config);
    // Keep this composition consistent with the apply-time injection (plate volume map, else
    // per-extruder defaults); a divergent volume map would silently mis-resolve per-filament reads.
    ...
    return preset_bundle->full_config(false, filament_maps, f_volume_maps);
}
